...
func connectDB(logger echo.Logger) (*sqlx.DB, error) {
	...
	conf.DBName = "isupipe"
	conf.ParseTime = true
	conf.InterpolateParams = true // ADMIN PREPARE対策
...
func connectDB2(logger echo.Logger) (*sqlx.DB, error) {
	const (
		networkTypeEnvKey = "ISUCON13_MYSQL_DIALCONFIG_NET"
...
func main() {
	...
	// DB接続2
	conn2, err2 := connectDB2(e.Logger)
	if err2 != nil {
		e.Logger.Errorf("failed to connect db: %v", err2)
		os.Exit(1)
	}
	defer conn2.Close()
	dbConn2 = conn2
